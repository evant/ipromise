...
/**
 * A promise is a way to return a result the will be fulfilled sometime in the future. This fixes the inversion of
 * control that callback-style functions creates and restores the composeability of return values.
 * <p/>
 ...
 * }</pre>
 * <p/>
 * With Promises, the asynchronous method looks much more like a standard method with a return value.
 * <pre>{@code
 ...
 * }</pre>
 * <p/>
 * In addition to creating a standard interface for all asynchronous functions, Promises can also more-robustly handle
 * error and cancellation situations.
 * <p/>
 * You cannot construct a {@code Promise} directly, instead you must get one from a {@link Deferred}.
 *
 * @author Evan Tatarka
 * @see Deferred
 */
public class Promise<T, E extends Exception> {
   ...
   /**
    * Constructs a new promise with the given {@link CancelToken}. When the token is canceled, the promise is also
    * canceled. This is used internally by {@link Deferred}.
    *
   ...
   /**
    * Delivers a result to all listeners of the {@code Promise}. This is used internally by {@link Deferred}.
    *
   ...
   /**
    * Cancels the {@code Promise}, notifying all listeners and propagating the cancellation to all Promises that share
    * the {@link CancelToken}.
    */
   ...
   /**
    * Listens to a {@code Promise}, getting a result when the {@code Promise} completes. If the {@code Promise} has
    * already completed, the listener is immediately called. This way you can't "miss" the result.
    *
   ...
   /**
    * Constructs a new promise that returns when the original promise returns but, passes the result through the given
    * {@link Promise.Map} function. If the result of the original {@code Promise} is not successful, the error or
    * cancellation is passed through to the new {@code Promise} without invoking the {@link Promise.Map} function. This
    * way you can do some work on an asynchronous result and pass it along without worrying about error or cancel
    * states.
    *
   ...
   /**
    * Constructs a new {@code Promise} that chains two promises in succession. If the first {@code Promise} is not
    * successful, the resulting {@code Promise} immediately receives the error or cancellation and the second {@code
    * Promise} is never called.
    *
    ...
    * @param <E2>  the type of the new {@code Promise} error result.The reason this is a separate type is to allow
    *              chaining of promises with different error types. Since the original error may be passed through,
    *              this must be a superclass of both the first and second {@code Promise} error types.
    * @return the new {@code Promise}
    */
   ...
   /**
    * This adapter is useful for when you only care about some of the possible result states or you want to split out
    * the possible result states in separate methods. If you override {@link Adapter#result(Result)}, you must call
    * {@code super} for the adapter to work correctly.
    *
    * @param <T> the success result type
    * @param <E> the error result type
    */
    ...
    * @param <A>  the type of the original {@code Promise} success result
    * @param <B>  the type of the second {@code Promise} success result
    * @param <ER> the type of the chain's error result. The must be a superclass of the error result of the first and
    *             second {@code Promise}.
   ...
   /**
    * The Exception thrown if a result has already been delivered and a second result is attempted to be delivered.
    */
